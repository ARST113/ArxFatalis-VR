...
struct OpenGLESGraphicsPlugin : public IGraphicsPlugin {
    ...
    void RenderView(std::shared_ptr<IApplication>& application, const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
        ...
        // The legacy GL4ES renderer can leave scissor and write masks set.
        // A plain glClear then only clears a fragment of a recycled OpenXR
        // swapchain image, which shows up as black/frozen wedges around menus
        // and during fast head turns. Clear the entire eye and restore the
        // exact state before handing control back to the engine.
