...
void CopyPose(ArxVrPose& target, const XrPosef& source) {
    target.orientation[0] = source.orientation.x;
    target.orientation[1] = source.orientation.y;
    target.orientation[2] = source.orientation.z;
    target.orientation[3] = source.orientation.w;
    target.position[0] = source.position.x;
    target.position[1] = source.position.y;
    target.position[2] = source.position.z;
}

ArxVrUpdateTrackingFn ResolveArxVrTrackingBridge() {
    static ArxVrUpdateTrackingFn updateTracking = nullptr;
    ...
    void* arxLibrary = dlopen("libarx.so", RTLD_NOW | RTLD_NOLOAD);
    if (!arxLibrary) {
        arxLibrary = dlopen("libarx.so", RTLD_NOW);
    }
    if (!arxLibrary) {
        const char* error = dlerror();
        Log::Write(Log::Level::Error, Fmt("ArxVR bridge: libarx.so unavailable: %s", error ? error : "unknown error"));
        return nullptr;
    }

    updateTracking = reinterpret_cast<ArxVrUpdateTrackingFn>(dlsym(arxLibrary, "arxvr_update_tracking"));
    if (!updateTracking) {
        const char* error = dlerror();
        Log::Write(Log::Level::Error, Fmt("ArxVR bridge: arxvr_update_tracking unavailable: %s", error ? error : "unknown error"));
        return nullptr;
    }

    Log::Write(Log::Level::Info, "ArxVR bridge resolved; streaming 6DoF head and controller state");
    return updateTracking;
}

using ArxVrRenderStateFn = unsigned (*)();

ArxVrRenderStateFn ResolveArxVrRenderState() {
    static ArxVrRenderStateFn renderState = nullptr;
    static bool resolutionAttempted = false;
    if (resolutionAttempted) {
        return renderState;
    }
    ...
    renderState = reinterpret_cast<ArxVrRenderStateFn>(
        dlsym(arxLibrary, "arxvr_engine_vr_render_state"));
    return renderState;
}
...
XrQuaternionf QuaternionFromYawPitch(float yawRadians, float pitchRadians) {
    const XrQuaternionf yaw{0.f, std::sin(yawRadians * 0.5f), 0.f,
                            std::cos(yawRadians * 0.5f)};
    const XrQuaternionf pitch{std::sin(pitchRadians * 0.5f), 0.f, 0.f,
                              std::cos(pitchRadians * 0.5f)};
    return QuaternionMultiply(yaw, pitch);
}

XrVector3f RotateVector(const XrQuaternionf& q, const XrVector3f& v) {
    const XrQuaternionf p{v.x, v.y, v.z, 0.f};
    const XrQuaternionf conjugate{-q.x, -q.y, -q.z, q.w};
    const XrQuaternionf rotated = QuaternionMultiply(QuaternionMultiply(q, p), conjugate);
...
struct OpenXrProgram : IOpenXrProgram {
    ...
    void InitializeActions() {
        ...
        // Suggest bindings for the PICO Controller.
        {
            ...
            // PICO 4 Ultra must always use its pico4s profile. The firmware ROM
            // query occasionally returns zero during a warm OpenXR restart;
            // treating that transient value as an old headset selected the Neo3
            // profile and left xrSuggestInteractionProfileBindings blocked on the
            // headset's permanent loading screen.
            if (m_deviceType != DeviceTypePico4Ultra
                && (m_deviceROM < 0x540 || m_deviceType == DeviceTypePico4)) {
                interactionProfilePath = "/interaction_profiles/pico/neo3_controller";
            }

            Log::Write(Log::Level::Info,
                       Fmt("ArxVR controller profile: %s device=%d rom=0x%x",
                           interactionProfilePath, int(m_deviceType), m_deviceROM));
...
//                                                            {m_input.menuAction, menuPath[Side::LEFT]}
            ...
            Log::Write(Log::Level::Info, "ArxVR controller bindings accepted");
        }
        ...
        Log::Write(Log::Level::Info, "ArxVR action spaces attached");
    }
    ...
    void InitializeSession() override {
        ...
        Log::Write(Log::Level::Info, "ArxVR view reference space created");

        {
            XrReferenceSpaceCreateInfo referenceSpaceCreateInfo = GetXrReferenceSpaceCreateInfo(m_options.AppSpace);
            m_appSpaceType = referenceSpaceCreateInfo.referenceSpaceType;
            CHECK_XRCMD(xrCreateReferenceSpace(m_session, &referenceSpaceCreateInfo, &m_appSpace));
        }
        Log::Write(Log::Level::Info, "ArxVR application reference space created");

        // Arx Fatalis is an opaque VR application. The original PICO sample created
        // passthrough layers and a triangle mesh unconditionally here, even while
        // passthrough was disabled. Some PICO 4 Ultra runtimes block in that sample
        // setup and never reach the game engine.
        Log::Write(Log::Level::Info, "ArxVR opaque mode: passthrough initialization skipped");

        // Visible ArxVR hands are driven by controller grip/aim poses and trigger
        // values. Optical hand tracking is unrelated and can likewise stall session
        // startup when the headset is in controller mode.
        Log::Write(Log::Level::Info, "ArxVR optical hand tracking skipped; controller hands active");
    }

    void InitializeApplication() override {
        ResolveArxVrTrackingBridge();
        ...
        while (const XrEventDataBaseHeader* event = TryReadNextEvent()) {
            switch (event->type) {
                ...
                case XR_TYPE_EVENT_DATA_REFERENCE_SPACE_CHANGE_PENDING: {
                    ...
                    if ((referenceSpaceChangePending.session == XR_NULL_HANDLE
                         || referenceSpaceChangePending.session == m_session)
                        && referenceSpaceChangePending.referenceSpaceType == m_appSpaceType) {
                        m_referenceSpaceChangePending = true;
                        m_referenceSpaceChangeTime = referenceSpaceChangePending.changeTime;
                        Log::Write(Log::Level::Info,
                                   Fmt("ArxVR application reference-space change scheduled for %lld",
                                       referenceSpaceChangePending.changeTime));
                    }
                    break;
                }
                default: {
    ...
    void HandleSessionStateChangedEvent(const XrEventDataSessionStateChanged& stateChangedEvent, bool* exitRenderLoop, bool* requestRestart) {
        ...
        if (m_sessionState != XR_SESSION_STATE_FOCUSED) {
            m_application->onSessionFocusLost();
        }
    ...
    void PollActions() override {
        ...
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            ...
            if (hand == Side::LEFT) {
                m_arxVrTracking.leftTrigger = applicationEvent.trigger;
                m_arxVrTracking.leftSqueeze = applicationEvent.squeeze;
                m_arxVrTracking.leftStick[0] = applicationEvent.thumbstick_x;
                m_arxVrTracking.leftStick[1] = applicationEvent.thumbstick_y;
                if (applicationEvent.click_trigger || applicationEvent.trigger >= 0.55f) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_TRIGGER;
                if (applicationEvent.click_squeeze || applicationEvent.squeeze >= 0.55f) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_SQUEEZE;
                if (applicationEvent.click_thumbstck) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_STICK;
                if (applicationEvent.click_x) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_X;
                if (applicationEvent.click_y) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_Y;
                if (applicationEvent.click_menu) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_MENU;
                if (applicationEvent.touch_trigger) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_TRIGGER_TOUCH;
                if (applicationEvent.touch_thumbstick) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_STICK_TOUCH;
            } else {
                m_arxVrTracking.rightTrigger = applicationEvent.trigger;
                m_arxVrTracking.rightSqueeze = applicationEvent.squeeze;
                m_arxVrTracking.rightStick[0] = applicationEvent.thumbstick_x;
                m_arxVrTracking.rightStick[1] = applicationEvent.thumbstick_y;
                if (applicationEvent.click_trigger || applicationEvent.trigger >= 0.55f) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_TRIGGER;
                if (applicationEvent.click_squeeze || applicationEvent.squeeze >= 0.55f) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_SQUEEZE;
                if (applicationEvent.click_thumbstck) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_STICK;
                if (applicationEvent.click_a) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_A;
                if (applicationEvent.click_b) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_B;
                if (applicationEvent.click_menu) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_MENU;
                if (applicationEvent.touch_trigger) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_TRIGGER_TOUCH;
                if (applicationEvent.touch_thumbstick) m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_STICK_TOUCH;
            }
        }
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews, XrCompositionLayerProjection& layer) {
        ...
        // A REFERENCE_SPACE_CHANGE_PENDING event can arrive before its stated
        // changeTime. Recenter only once xrLocateViews is sampling the new
        // application space, so Arx never mistakes a runtime origin jump for
        // a physical 180-degree head/body turn.
        if (m_referenceSpaceChangePending
            && predictedDisplayTime >= m_referenceSpaceChangeTime) {
            m_application->onReferenceSpaceChanged();
            m_referenceSpaceChangePending = false;
            Log::Write(Log::Level::Info,
                       Fmt("ArxVR application reference-space change applied at %lld",
        ...
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            ...
            if (XR_UNQUALIFIED_SUCCESS(res)) {
                if ((spaceLocation.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0 &&
                    ...
                    if (hand == Side::LEFT) {
                        CopyPose(m_arxVrTracking.leftAim, spaceLocation.pose);
                        m_arxVrTracking.validMask |= ARXVR_VALID_LEFT_AIM;
                    } else {
                        CopyPose(m_arxVrTracking.rightAim, spaceLocation.pose);
                        m_arxVrTracking.validMask |= ARXVR_VALID_RIGHT_AIM;
                    }
                }
            }

            XrSpaceLocation gripLocation{XR_TYPE_SPACE_LOCATION};
            res = xrLocateSpace(m_input.handSpace[hand], m_appSpace, predictedDisplayTime, &gripLocation);
            CHECK_XRRESULT(res, "xrLocateSpace(grip)");
            if (XR_UNQUALIFIED_SUCCESS(res) &&
                ...
                // The mesh represents the user's hand around the controller,
                // so attach it to the physical grip pose. The independent aim
                // pose remains the source for pointing and world interaction.
                m_application->setControllerGripPose(int(hand), gripLocation.pose);
                if (hand == Side::LEFT) {
                    CopyPose(m_arxVrTracking.leftGrip, gripLocation.pose);
                    m_arxVrTracking.validMask |= ARXVR_VALID_LEFT_GRIP;
                } else {
                    CopyPose(m_arxVrTracking.rightGrip, gripLocation.pose);
                    m_arxVrTracking.validMask |= ARXVR_VALID_RIGHT_GRIP;
                }
            }
        }
        ...
        if (XR_UNQUALIFIED_SUCCESS(res) &&
            ...
            (spaceLocation.locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) != 0) {
            CopyPose(m_arxVrTracking.head, spaceLocation.pose);
            m_arxVrTracking.validMask |= ARXVR_VALID_HEAD;
        } else {
            CopyPose(m_arxVrTracking.head, m_views[0].pose);
            if (viewCountOutput > 1) {
                m_arxVrTracking.head.position[0] = (m_views[0].pose.position.x + m_views[1].pose.position.x) * 0.5f;
                m_arxVrTracking.head.position[1] = (m_views[0].pose.position.y + m_views[1].pose.position.y) * 0.5f;
                m_arxVrTracking.head.position[2] = (m_views[0].pose.position.z + m_views[1].pose.position.z) * 0.5f;
            }
            m_arxVrTracking.validMask |= ARXVR_VALID_HEAD;
        }
        ...
        // ADB key events do not enter the OpenXR action set. Keep this narrow
        // diagnostics-only hook so the automated headset test can exercise the
        // exact B/Menu path consumed by the engine, including press/release
        // edges, without changing ordinary controller input.
        char forcedMenuValue[PROP_VALUE_MAX] = {};
        __system_property_get("debug.arxvr.force_menu", forcedMenuValue);
        const bool forcedMenuPressed = strcmp(forcedMenuValue, "1") == 0;
        static bool previousForcedMenuPressed = false;
        if (forcedMenuPressed) {
            m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_B;
        }
        if (forcedMenuPressed != previousForcedMenuPressed) {
            Log::Write(Log::Level::Info,
                       Fmt("ARXVR_MENU_TEST forced_button=%s",
                           forcedMenuPressed ? "down" : "up"));
            previousForcedMenuPressed = forcedMenuPressed;
        }

        ++m_arxVrTracking.frameNumber;
        XrPosef currentHead{};
        currentHead.position = {m_arxVrTracking.head.position[0],
                                m_arxVrTracking.head.position[1],
                                m_arxVrTracking.head.position[2]};
        currentHead.orientation = {m_arxVrTracking.head.orientation[0],
                                   m_arxVrTracking.head.orientation[1],
                                   m_arxVrTracking.head.orientation[2],
                                   m_arxVrTracking.head.orientation[3]};
        m_application->setTrackingFrameHead(currentHead, IsSessionFocused());
        if (ArxVrUpdateTrackingFn updateTracking = ResolveArxVrTrackingBridge()) {
            updateTracking(&m_arxVrTracking);
            if (!m_loggedArxVrTrackingFrame) {
                Log::Write(Log::Level::Info,
                           Fmt("ArxVR tracking frame=%llu validMask=0x%02x buttonMask=0x%04x",
                               static_cast<unsigned long long>(m_arxVrTracking.frameNumber),
                               m_arxVrTracking.validMask, m_arxVrTracking.buttonMask));
                m_loggedArxVrTrackingFrame = true;
            }
        }
    ...
    void applyArxVrQaInput() {
        char text[PROP_VALUE_MAX] = {};
        __system_property_get("debug.arxvr.qa_input", text);
        ArxVrQaInput command;
        if (!ParseArxVrQaInput(text, command)) {
            if (m_qaActive) {
                m_qaActive = false;
                m_application->onReferenceSpaceChanged();
                Log::Write(Log::Level::Info, "ARXVR_QA disabled; physical tracking restored");
            }
            return;
        }
        const auto now = std::chrono::steady_clock::now();
        if (!m_qaActive) {
            m_qaBase = m_arxVrTracking.head;
            const XrQuaternionf raw{m_qaBase.orientation[0], m_qaBase.orientation[1],
                                    m_qaBase.orientation[2], m_qaBase.orientation[3]};
            const XrVector3f forward = RotateVector(raw, {0.f, 0.f, -1.f});
            const XrQuaternionf upright = QuaternionFromYawPitch(
                std::atan2(-forward.x, -forward.z), 0.f);
            m_qaBase.orientation[0] = upright.x; m_qaBase.orientation[1] = upright.y;
            m_qaBase.orientation[2] = upright.z; m_qaBase.orientation[3] = upright.w;
            m_qaActive = true;
            m_qaSequence = 0;
            m_application->onReferenceSpaceChanged();
            Log::Write(Log::Level::Info, "ARXVR_QA enabled: SYNTHETIC head/controllers; no world-state shortcuts");
        }
        if (command.sequence != m_qaSequence) {
            m_qaSequence = command.sequence;
            m_qaCommandTime = now;
            Log::Write(Log::Level::Info, Fmt("ARXVR_QA command=%s", text));
        }
        const float age = std::chrono::duration<float>(now - m_qaCommandTime).count();
        const bool pulse = age < command.duration;
        const XrQuaternionf base{m_qaBase.orientation[0], m_qaBase.orientation[1],
                                 m_qaBase.orientation[2], m_qaBase.orientation[3]};
        const XrQuaternionf upright = QuaternionMultiply(base,
            QuaternionFromYawPitch(glm::radians(command.yaw), 0.f));
        XrPosef head{QuaternionMultiply(upright,
            QuaternionFromYawPitch(0.f, glm::radians(command.pitch))),
            {m_qaBase.position[0], m_qaBase.position[1] + command.height, m_qaBase.position[2]}};
        const XrVector3f rightOffset = RotateVector(upright,
            {command.handX, command.handY, command.handZ});
        XrPosef right{QuaternionMultiply(upright,
            QuaternionFromYawPitch(glm::radians(command.handYaw), glm::radians(command.handPitch))),
            {head.position.x + rightOffset.x, head.position.y + rightOffset.y,
             head.position.z + rightOffset.z}};
        const XrVector3f leftOffset = RotateVector(upright, {-0.2f, -0.25f, -0.35f});
        XrPosef left{upright, {head.position.x + leftOffset.x, head.position.y + leftOffset.y,
                              head.position.z + leftOffset.z}};
        CopyPose(m_arxVrTracking.head, head);
        CopyPose(m_arxVrTracking.rightAim, right); CopyPose(m_arxVrTracking.rightGrip, right);
        CopyPose(m_arxVrTracking.leftAim, left); CopyPose(m_arxVrTracking.leftGrip, left);
        m_arxVrTracking.validMask = ARXVR_VALID_HEAD | ARXVR_VALID_LEFT_AIM
            | ARXVR_VALID_LEFT_GRIP | ARXVR_VALID_RIGHT_AIM | ARXVR_VALID_RIGHT_GRIP;
        m_arxVrTracking.buttonMask = pulse ? command.buttons : 0u;
        m_arxVrTracking.leftStick[0] = pulse ? command.moveX : 0.f;
        m_arxVrTracking.leftStick[1] = pulse ? command.moveY : 0.f;
        m_arxVrTracking.rightStick[0] = m_arxVrTracking.rightStick[1] = 0.f;
        m_arxVrTracking.leftTrigger = pulse && (command.buttons & ARXVR_BUTTON_LEFT_TRIGGER) ? 1.f : 0.f;
        m_arxVrTracking.leftSqueeze = pulse && (command.buttons & ARXVR_BUTTON_LEFT_SQUEEZE) ? 1.f : 0.f;
        m_arxVrTracking.rightTrigger = pulse && (command.buttons & ARXVR_BUTTON_RIGHT_TRIGGER) ? 1.f : 0.f;
        m_arxVrTracking.rightSqueeze = pulse && (command.buttons & ARXVR_BUTTON_RIGHT_SQUEEZE) ? 1.f : 0.f;
        m_application->setControllerPose(Side::LEFT, left);
        m_application->setControllerGripPose(Side::LEFT, left);
        m_application->setControllerPose(Side::RIGHT, right);
        m_application->setControllerGripPose(Side::RIGHT, right);
        ApplicationEvent leftEvent{}, rightEvent{};
        leftEvent.trigger = m_arxVrTracking.leftTrigger;
        leftEvent.squeeze = m_arxVrTracking.leftSqueeze;
        rightEvent.trigger = m_arxVrTracking.rightTrigger;
        rightEvent.squeeze = m_arxVrTracking.rightSqueeze;
        m_application->inputEvent(Side::LEFT, leftEvent);
        m_application->inputEvent(Side::RIGHT, rightEvent);
    }

    void applyArxVrHumanTest() {
        if (!m_arxVrHumanTestChecked) {
            char value[PROP_VALUE_MAX] = {};
            __system_property_get("debug.arxvr.auto_human", value);
            m_arxVrHumanTestEnabled = strcmp(value, "1") == 0;
            char routeValue[PROP_VALUE_MAX] = {};
            __system_property_get("debug.arxvr.auto_traverse", routeValue);
            m_arxVrRouteTestEnabled = strcmp(routeValue, "1") == 0;
            m_arxVrHumanTestChecked = true;
            if (m_arxVrHumanTestEnabled) {
                Log::Write(Log::Level::Info,
                           "ARXVR_HUMAN armed; waiting for real stereo gameplay");
            }
        }
        if (!m_arxVrHumanTestEnabled) {
            return;
        ...
        ArxVrRenderStateFn renderState = ResolveArxVrRenderState();
        const unsigned state = renderState ? renderState() : 0u;
        const auto now = std::chrono::steady_clock::now();

        if (!m_arxVrHumanTestActive) {
            if ((state & 0x100u) == 0u || (state & 0x040u) == 0u) {
                return;
            }
            m_arxVrHumanTestBase = m_arxVrTracking;
            m_arxVrHumanTestStart = now;
            m_arxVrHumanTestActive = true;
            Log::Write(Log::Level::Info,
                       Fmt("ARXVR_HUMAN phase=0 name=settle state=0x%03x", state));
        } else if ((state & 0x100u) == 0u || (state & 0x040u) == 0u) {
            if (!m_arxVrHumanTestPaused) {
                m_arxVrHumanTestPaused = true;
                m_arxVrHumanTestPauseStart = now;
                Log::Write(Log::Level::Info,
                           Fmt("ARXVR_HUMAN pause=non_stereo state=0x%03x", state));
            }
            return;
        } else if (m_arxVrHumanTestPaused) {
            m_arxVrHumanTestStart += now - m_arxVrHumanTestPauseStart;
            m_arxVrHumanTestPaused = false;
            Log::Write(Log::Level::Info,
                       Fmt("ARXVR_HUMAN resume=stereo state=0x%03x", state));
        }

        const float seconds = std::chrono::duration<float>(
            now - m_arxVrHumanTestStart).count();
        int phase = 0;
        const char* phaseName = "settle";
        float yaw = 0.f;
        float pitch = 0.f;
        XrVector3f translation{0.f, 0.f, 0.f};
        float handWave = 0.f;
        ...
        if (seconds < 2.f) {
            ...
            phase = 1;
            phaseName = "yaw_360";
            yaw = 2.f * static_cast<float>(M_PI) * Clamp01((seconds - 2.f) / 4.f);
        } else if (seconds < 7.f) {
            phase = 2;
            phaseName = "yaw_return_settle";
        } else if (seconds < 8.5f) {
            phase = 3;
            phaseName = "pitch_up";
            pitch = glm::radians(55.f) * Clamp01((seconds - 7.f) / 1.5f);
        } else if (seconds < 11.5f) {
            phase = 4;
            phaseName = "pitch_up_to_down";
            pitch = glm::radians(55.f - 110.f * Clamp01((seconds - 8.5f) / 3.f));
        } else if (seconds < 13.f) {
            phase = 5;
            phaseName = "pitch_return";
            pitch = glm::radians(-55.f + 55.f * Clamp01((seconds - 11.5f) / 1.5f));
        } else if (seconds < 17.f) {
            phase = 6;
            phaseName = "head_6dof";
            const float t = (seconds - 13.f) / 4.f * 2.f * static_cast<float>(M_PI);
            translation = {0.22f * std::sin(t), 0.42f * std::sin(t * 2.f),
                           0.20f * (std::cos(t) - 1.f)};
        } else if (seconds < 24.f) {
            phase = 7;
            phaseName = "hands_and_actions";
            handWave = (seconds - 17.f) / 7.f * 2.f * static_cast<float>(M_PI);
        } else {
            phase = 8;
            phaseName = m_arxVrRouteTestEnabled ? "route_periodic_crouch" : "complete";
            m_arxVrHumanTestComplete = true;
            if (m_arxVrRouteTestEnabled) {
                // Continue a complete stand -> crouch -> stand cycle until the
                // route reaches the trapdoor. This remains a real 6DoF HMD
                // height signal, so the engine must enter and leave its native
                // PLAYER_CROUCH state rather than receiving a debug crouch key.
                const float cycle = std::fmod(seconds - 24.f, 6.f);
                if (cycle < 1.f) {
                    translation.y = 0.f;
                } else if (cycle < 2.f) {
                    translation.y = -0.34f * Clamp01(cycle - 1.f);
                } else if (cycle < 4.f) {
                    translation.y = -0.34f;
                } else if (cycle < 5.f) {
                    translation.y = -0.34f * (1.f - Clamp01(cycle - 4.f));
                } else {
                    translation.y = 0.f;
                }
            }
        ...
        const XrQuaternionf baseOrientation{
            m_arxVrHumanTestBase.head.orientation[0],
            m_arxVrHumanTestBase.head.orientation[1],
            m_arxVrHumanTestBase.head.orientation[2],
            m_arxVrHumanTestBase.head.orientation[3]
        };
        const XrQuaternionf localRotation = QuaternionFromYawPitch(yaw, pitch);
        const XrQuaternionf headOrientation = QuaternionMultiply(baseOrientation, localRotation);
        const XrVector3f rotatedTranslation = RotateVector(baseOrientation, translation);
        XrPosef head{};
        head.orientation = headOrientation;
        head.position = {
            m_arxVrHumanTestBase.head.position[0] + rotatedTranslation.x,
            m_arxVrHumanTestBase.head.position[1] + rotatedTranslation.y,
            m_arxVrHumanTestBase.head.position[2] + rotatedTranslation.z
        };
        CopyPose(m_arxVrTracking.head, head);

        const float wave = std::sin(handWave);
        const XrVector3f leftLocal{-0.18f, 0.05f + 0.08f * wave, -0.38f};
        const XrVector3f rightLocal{0.18f, 0.05f - 0.08f * wave, -0.38f};
        const XrVector3f leftOffset = RotateVector(headOrientation, leftLocal);
        const XrVector3f rightOffset = RotateVector(headOrientation, rightLocal);
        XrPosef leftHand{headOrientation,
                         {head.position.x + leftOffset.x, head.position.y + leftOffset.y,
                          head.position.z + leftOffset.z}};
        XrPosef rightHand{headOrientation,
                          {head.position.x + rightOffset.x, head.position.y + rightOffset.y,
                           head.position.z + rightOffset.z}};
        CopyPose(m_arxVrTracking.leftAim, leftHand);
        CopyPose(m_arxVrTracking.leftGrip, leftHand);
        CopyPose(m_arxVrTracking.rightAim, rightHand);
        CopyPose(m_arxVrTracking.rightGrip, rightHand);
        m_arxVrTracking.validMask |= ARXVR_VALID_HEAD | ARXVR_VALID_LEFT_AIM
                                  | ARXVR_VALID_LEFT_GRIP | ARXVR_VALID_RIGHT_AIM
                                  | ARXVR_VALID_RIGHT_GRIP;
        m_application->setControllerPose(Side::LEFT, leftHand);
        m_application->setControllerPose(Side::RIGHT, rightHand);
        m_application->setControllerGripPose(Side::LEFT, leftHand);
        m_application->setControllerGripPose(Side::RIGHT, rightHand);

        m_arxVrTracking.buttonMask = 0;
        m_arxVrTracking.leftTrigger = 0.f;
        m_arxVrTracking.leftSqueeze = 0.f;
        m_arxVrTracking.rightTrigger = 0.f;
        m_arxVrTracking.rightSqueeze = 0.f;
        m_arxVrTracking.leftStick[0] = 0.f;
        m_arxVrTracking.leftStick[1] = 0.f;
        m_arxVrTracking.rightStick[0] = 0.f;
        m_arxVrTracking.rightStick[1] = 0.f;
        if (phase == 7) {
            const float actionTime = seconds - 17.f;
            if (actionTime >= 0.25f && actionTime < 3.25f) {
                // The physical lower grip is the primary world interaction.
                m_arxVrTracking.rightSqueeze = 1.f;
                m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_SQUEEZE;
            } else if (actionTime >= 3.5f && actionTime < 4.5f) {
                m_arxVrTracking.rightTrigger = 1.f;
                m_arxVrTracking.buttonMask |= ARXVR_BUTTON_RIGHT_TRIGGER
                                           | ARXVR_BUTTON_RIGHT_TRIGGER_TOUCH;
            } else if (actionTime >= 4.8f && actionTime < 5.8f) {
                m_arxVrTracking.leftSqueeze = 1.f;
                m_arxVrTracking.buttonMask |= ARXVR_BUTTON_LEFT_SQUEEZE;
            }
        }

        ApplicationEvent leftHandEvent{};
        leftHandEvent.trigger = m_arxVrTracking.leftTrigger;
        leftHandEvent.squeeze = m_arxVrTracking.leftSqueeze;
        ApplicationEvent rightHandEvent{};
        rightHandEvent.trigger = m_arxVrTracking.rightTrigger;
        rightHandEvent.squeeze = m_arxVrTracking.rightSqueeze;
        m_application->inputEvent(Side::LEFT, leftHandEvent);
        m_application->inputEvent(Side::RIGHT, rightHandEvent);

        if (phase != m_arxVrHumanTestPhase) {
            m_arxVrHumanTestPhase = phase;
            Log::Write(Log::Level::Info,
                       Fmt("ARXVR_HUMAN phase=%d name=%s t=%.2f yaw=%.1f pitch=%.1f "
                           "head=(%.3f,%.3f,%.3f) hands=(%.3f,%.3f,%.3f)/(%.3f,%.3f,%.3f)",
                           phase, phaseName, seconds, glm::degrees(yaw), glm::degrees(pitch),
        ...
        if (phase == 1) {
            const int checkpoint = std::min(4, static_cast<int>(glm::degrees(yaw) / 90.f));
            if (checkpoint > m_arxVrHumanYawCheckpoint) {
                m_arxVrHumanYawCheckpoint = checkpoint;
                Log::Write(Log::Level::Info,
                           Fmt("ARXVR_HUMAN checkpoint=yaw_%03d", checkpoint * 90));
            }
        }
        if (phase == 7) {
            const float actionTime = seconds - 17.f;
            int actionStep = 0;
            const char* actionName = "hands_move";
            if (actionTime >= 0.25f && actionTime < 3.25f) {
                actionStep = 1;
                actionName = "right_lower_grip_interact";
            } else if (actionTime >= 3.25f && actionTime < 3.5f) {
                actionStep = 2;
                actionName = "right_lower_grip_release";
            } else if (actionTime >= 3.5f && actionTime < 4.5f) {
                actionStep = 3;
                actionName = "right_trigger_rune";
            } else if (actionTime >= 4.5f && actionTime < 4.8f) {
                actionStep = 4;
                actionName = "right_trigger_release";
            } else if (actionTime >= 4.8f && actionTime < 5.8f) {
                actionStep = 5;
                actionName = "left_squeeze_magic";
            } else if (actionTime >= 5.8f) {
                actionStep = 6;
                actionName = "left_squeeze_release";
            }
            if (actionStep != m_arxVrHumanActionStep) {
                m_arxVrHumanActionStep = actionStep;
                Log::Write(Log::Level::Info,
                           Fmt("ARXVR_HUMAN action=%s", actionName));
            }
        }
    }
   ...
   private:
    ...
    XrReferenceSpaceType m_appSpaceType{XR_REFERENCE_SPACE_TYPE_LOCAL};
    bool m_referenceSpaceChangePending{false};
    XrTime m_referenceSpaceChangeTime{0};
    ...
    ArxVrTrackingState m_arxVrTracking = {};
    bool m_qaActive = false;
    unsigned m_qaSequence = 0;
    ArxVrPose m_qaBase{};
    std::chrono::steady_clock::time_point m_qaCommandTime{};
    bool m_loggedArxVrTrackingFrame{false};
    bool m_arxVrHumanTestChecked{false};
    bool m_arxVrHumanTestEnabled{false};
    bool m_arxVrRouteTestEnabled{false};
    bool m_arxVrHumanTestActive{false};
    bool m_arxVrHumanTestComplete{false};
    bool m_arxVrHumanTestPaused{false};
    int m_arxVrHumanTestPhase{-1};
    int m_arxVrHumanYawCheckpoint{0};
    int m_arxVrHumanActionStep{-1};
    std::chrono::steady_clock::time_point m_arxVrHumanTestStart{};
    std::chrono::steady_clock::time_point m_arxVrHumanTestPauseStart{};
    ArxVrTrackingState m_arxVrHumanTestBase{};
