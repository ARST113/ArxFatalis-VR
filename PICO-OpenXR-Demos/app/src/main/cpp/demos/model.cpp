...
static glm::mat4 assimpToGlm(const aiMatrix4x4& source) {
    glm::mat4 result(1.0f);
    result[0][0] = source.a1; result[1][0] = source.a2;
    result[2][0] = source.a3; result[3][0] = source.a4;
    result[0][1] = source.b1; result[1][1] = source.b2;
    result[2][1] = source.b3; result[3][1] = source.b4;
    result[0][2] = source.c1; result[1][2] = source.c2;
    result[2][2] = source.c3; result[3][2] = source.c4;
    result[0][3] = source.d1; result[1][3] = source.d2;
    result[2][3] = source.d3; result[3][3] = source.d4;
    return result;
}
    ...
    if (init) {
        ...
        const char* fragmentShaderCode = R"_(
            ...
            void main()
            {
                ...
                // colorTint.rgb is a material multiplier.  The original hand
                // FBX files reference a pale skin material; multiplying the
                // explicitly selected diffuse map prevents that fallback from
                // turning the VR gloves into flat peach silhouettes.
                vec3 material = sampled.rgb * colorTint.rgb;
                vec3 lightDirection = normalize(vec3(-0.35, 0.75, 0.55));
                float lighting = 0.48 + 0.52 * max(dot(normalize(WorldNormal), lightDirection), 0.0);
                FragColor = vec4(material * lighting, sampled.a);
            }
        )_";
...
void Model::processMeshBone(aiMesh* mesh, std::vector<Vertex>& vertices) {
    ...
    for (uint32_t i = 0; i < mesh->mNumBones; i++) {
        ...
        const glm::mat4 offset = assimpToGlm(mesh->mBones[i]->mOffsetMatrix);
        std::shared_ptr<boneInfo> boneInformation(new boneInfo(boneIndex, offset));
        const glm::mat4 bind = glm::inverse(offset);
        infof("ArxVR bone bind: %s pivot=(%.4f,%.4f,%.4f) axisX=(%.4f,%.4f,%.4f) axisY=(%.4f,%.4f,%.4f) axisZ=(%.4f,%.4f,%.4f)",
              name.c_str(), bind[3].x, bind[3].y, bind[3].z,
              bind[0].x, bind[0].y, bind[0].z,
              bind[1].x, bind[1].y, bind[1].z,
              bind[2].x, bind[2].y, bind[2].z);
        auto it = mBoneInfoMap.find(name);
        if (it == mBoneInfoMap.end()) {
            mBoneInfoMap[name] = boneInformation;
            if (mBoneNodeMatrices.size() <= static_cast<size_t>(boneIndex)) {
                mBoneNodeMatrices.resize(static_cast<size_t>(boneIndex) + 1u,
                                         glm::mat4(1.0f));
            }
        } else {
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    for (uint32_t i = 0; i < mesh->mNumVertices; i++) {
        ...
        if (!mHasBounds) {
            ...
            mBoundsMin = glm::min(mBoundsMin, vector);
            mBoundsMax = glm::max(mBoundsMax, vector);
        }
...
bool Model::render(const glm::mat4& p, const glm::mat4& v, const glm::mat4& m) {
    mShader.use();
    // The shader object is shared, but each hand has its own pose. Upload this
    // model's bone state immediately before drawing so one hand cannot overwrite
    // the other hand's finger curls.
    for (const auto& bone : mBoneInfoMap) {
        const int index = bone.second->id;
        if (index >= 0 && static_cast<size_t>(index) < mBoneNodeMatrices.size()) {
            const std::string uniform = "finalBoneNodesMatrices["
                                      + std::to_string(index) + "]";
            mShader.setUniformMat4(uniform, mBoneNodeMatrices[index]);
        }
    }
    mShader.setUniformMat4("projection", p);
    mShader.setUniformMat4("view", v);
    mShader.setUniformMat4("model", m);
    mShader.setUniformVec4("colorTint", mColorTint);
    draw();
    glUseProgram(0);
    return true;
}
...
void Model::initializeBoneNode() {
    size_t count = 0u;
    for (const auto& bone : mBoneInfoMap) {
        count = std::max(count, static_cast<size_t>(bone.second->id + 1));
    }
    mBoneNodeMatrices.assign(count, glm::mat4(1.0f));
}
...
void Model::setBoneNodeMatrices(const std::string& bone, const glm::mat4& m) {
    ...
    mBoneNodeMatrices[index] = m;
}

void Model::setBoneLocalRotation(const std::string& bone, const glm::mat4& rotation) {
    const glm::mat4 matrix = boneLocalRotationMatrix(bone, rotation);
    auto it = mBoneInfoMap.find(bone);
    if (it == mBoneInfoMap.end()) {
        return;
    }
    ...
    mBoneNodeMatrices[index] = matrix;
}

glm::mat4 Model::boneLocalRotationMatrix(const std::string& bone,
                                         const glm::mat4& rotation) const {
    auto it = mBoneInfoMap.find(bone);
    if (it == mBoneInfoMap.end()) {
        errorf("not found bone %s", bone.c_str());
        return glm::mat4(1.0f);
    }

    // Assimp's offset matrix is inverse(bindGlobal). Conjugating a local
    // rotation by the bind transform rotates the influenced vertices about the
    // authored joint pivot instead of orbiting the whole finger around origin.
    const glm::mat4& offset = it->second->offset;
    return glm::inverse(offset) * rotation * offset;
}
