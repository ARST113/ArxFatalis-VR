...
typedef struct Extentions_tag{
    ...
    // Arx Fatalis renders an opaque world. Starting PICO passthrough here opens
    // the system passthrough activity, steals VR focus during swapchain setup,
    // and can trigger a NativeActivity input-dispatch ANR before the first frame.
...
public:
    ...
    virtual void setControllerGripPose(int leftright, const XrPosef& pose) = 0;
    ...
    virtual void setTrackingFrameHead(const XrPosef& pose, bool focused) = 0;
    virtual void onSessionFocusLost() = 0;
