param(
    ...
    [string]$OutputDirectory = 'F:\CODEX\ArxFatalis VR\diagnostics\performance'
)
...
Write-Host 'ARXVR_PERF_TEST installing without clearing game data'
...
try {
    ...
    while([DateTime]::UtcNow -lt $deadline -and -not $complete) {
        ...
        $complete = $log.Contains('ARXVR_HUMAN phase=8 name=complete')
    ...
    Invoke-Adb shell screencap -p /sdcard/Pictures/arxvr-performance-complete.png | Out-Null
    Invoke-Adb pull /sdcard/Pictures/arxvr-performance-complete.png `
        (Join-Path $OutputDirectory 'complete.png') | Out-Null

    $metricLines = @(($finalLog -split "`r?`n") | Where-Object {
        $_ -match 'PxrMetric: FPS=.*Pkg=com\.arxvr\.android|ARXVR_(PERF|SCENE_PERF|UPDATE_PERF|RENDER_PERF)|ARXVR_HUMAN phase=8'
    })
    [IO.File]::WriteAllText((Join-Path $OutputDirectory 'performance.txt'),
                            ($metricLines -join "`r`n"), [Text.UTF8Encoding]::new($false))
    $metricLines | Select-Object -Last 30 | Write-Host
} finally {
    Write-Host 'ARXVR_PERF_TEST stopping application and resetting diagnostics'
