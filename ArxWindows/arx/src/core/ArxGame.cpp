...
void ArxGame::render() {
	...
	if(ARXmenu.mode() != Mode_InGame) {
		...
		if(g_openXR.isInitialized() && g_camera) {
			Camera * desktopCamera = g_camera;
			renderedInVR = g_openXR.renderFrame(*desktopCamera, [this](VREyeRenderInfo & eye) {
				PrepareCamera(&eye.camera, eye.viewport, eye.worldToView, eye.viewToClip);
				renderLevel();
			});
			PrepareCamera(desktopCamera, g_size);
		}
		if(!renderedInVR) {
			renderLevel();
		}
...
void ArxGame::onRendererInit(Renderer & renderer) {
	...
	#if ARX_HAVE_OPENXR
	if(g_openXRRequested && g_openXR.initialize()) {
		// xrWaitFrame is the authoritative frame pacer in VR. Desktop VSync or
		// the regular FPS limiter would add an avoidable second wait.
