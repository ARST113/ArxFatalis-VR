...
void PrepareCamera(Camera * cam, const Rect & viewport,
	               const glm::mat4x4 & worldToView,
	               const glm::mat4x4 & viewToClip) {

	SetActiveCamera(cam);

	g_preparedCamera.m_worldToView = worldToView;
	g_preparedCamera.m_viewToClip = viewToClip;
	GRenderer->SetViewMatrix(g_preparedCamera.m_worldToView);
	GRenderer->SetProjectionMatrix(g_preparedCamera.m_viewToClip);
	...
	ndcToScreen = glm::scale(ndcToScreen, Vec3f(1.f, -1.f, 1.f));
	g_preparedCamera.m_viewToScreen = ndcToScreen * g_preparedCamera.m_viewToClip;
	g_preparedCamera.m_worldToScreen = g_preparedCamera.m_viewToScreen
	                                 * g_preparedCamera.m_worldToView;
	g_preparedCamera.m_viewToWorld = glm::inverse(g_preparedCamera.m_worldToView);

	GRenderer->SetViewport(viewport);

	// The direction (0, 0, +Z) can be off-center for an HMD eye. Convert its
	// NDC offset to the screen-space center used by legacy picking helpers.
	const Vec2f ndcCenter(g_preparedCamera.m_viewToClip[2][0],
	                      g_preparedCamera.m_viewToClip[2][1]);
	const Vec2f viewportSize(viewport.size());
	g_preparedCamera.m_center = Vec2f(viewport.topLeft())
	                          + Vec2f((ndcCenter.x + 1.f) * viewportSize.x * 0.5f,
	                                  (1.f - ndcCenter.y) * viewportSize.y * 0.5f);
	Vec2f defov = Vec2f(g_preparedCamera.m_viewToClip[0][0],
	                    -g_preparedCamera.m_viewToClip[1][1]);
	g_preparedCamera.m_screenToView = 2.f / (viewportSize * defov);
}
