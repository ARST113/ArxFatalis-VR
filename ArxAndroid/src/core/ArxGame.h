...
public:
	...
	void run() override;
	bool initializeVr(RenderWindow * window);
	void frameVr(int eye, float verticalFovRadians);
	void openVrMainMenu();
	void startVrTraversalDiagnostic(std::string_view gripTarget = "stone");
	unsigned vrRenderState() const;
	bool shouldRenderVrStereo() const;
	void shutdownVr();
	
private:
	void updateTime();
	void updateInput();
	void updateVrTraversalDiagnostic();
	...
	void updateLevel();
	void renderLevel(bool secondaryVrEye = false);
	void renderVrEye(int eye, float verticalFovRadians);
