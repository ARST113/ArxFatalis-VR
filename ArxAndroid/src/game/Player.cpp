...
#if defined(ARXVR_ANDROID_BUILD)
	// Physical room-scale crouching must shrink collision immediately. Waiting
	// for the legacy crouch-start animation left the camera low while the player
	// body remained standing and could not pass beneath geometry.
	if((player.m_currentMovement & PLAYER_CROUCH) && arxvrPhysicalCrouchActive()
	   && !player.levitate) {
