...
void ARX_SPELLS_ManageMagic() {
	...
	bool magicModePressed = GInput->actionPressed(CONTROLS_CUST_MAGICMODE);
#if defined(ARXVR_ANDROID_BUILD)
	magicModePressed = magicModePressed || arxvrIsMagicModePressed();
#endif
...
#if defined(ARXVR_ANDROID_BUILD)
			runeDrawPressed = runeDrawPressed || arxvrIsRuneDrawing();
#endif
			if(!runeDrawPressed && ARX_FLARES_broken == 0) {
				ARX_FLARES_broken = 2;
