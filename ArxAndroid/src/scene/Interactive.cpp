...
void RenderInter() {
	...
	for(Entity & entity : entities.inScene()) {
...
#if defined(ARXVR_ANDROID_BUILD)
		// The original treat zone is much larger than the useful dungeon view
		// distance and transforms every object in it. Cull remote entities before
		// skeletal/static rendering; level scripts and physics keep updating.
		// Keep characters and useful props visible across a normal room, but do
		// not transform every decorative entity in the 18-24 m treat zone twice
		// per VR frame. The broad v32 distances caused a CPU fall from 72 to
		// 17-31 fps in the goblin rooms even though the GPU stayed near 7 ms.
		const float vrEntityDistance = (entity.ioflags & IO_NPC) ? 1600.f
		                             : (entity.ioflags & (IO_ITEM | IO_MOVABLE)) ? 1200.f
		                             : 900.f;
		if(fartherThan(entity.pos, g_camera->m_pos, vrEntityDistance)) {
			continue;
		}
		// The legacy path normally transforms every vertex before portal/frustum
		// rejection. A generous coarse sphere rejects off-screen objects first,
		// which is especially important during fast tracked-head movement.
		if(!VisibleSphere(Sphere(entity.pos - Vec3f(0.f, 60.f, 0.f), 240.f))) {
			continue;
		}
#endif
		...
		if(entity.animlayer[0].cur_anim) {
			...
			#if defined(ARXVR_ANDROID_BUILD)
			// World-space skinned vertices are shared by the stereo pair, but their
			// clip positions are eye-dependent. The legacy path reused the left-eye
			// projection for NPCs in the right eye.
			if(arxvrIsSecondaryEyePass()) {
			...
			if(!(entity.ioflags & IO_NPC) && entity.obj) {
				...
				#if defined(ARXVR_ANDROID_BUILD)
				if(arxvrIsSecondaryEyePass()) {
					// Reuse the primary eye's model transform. Re-projecting is still
					// required for correct stereo, but transforming every static vertex a
					// second time was a large CPU cost in prop-heavy goblin rooms.
					DrawEERIEInter_ViewProjectTransform(entity.obj);
					entity.bbox2D = UpdateBbox2d(*entity.obj);
					if(!ARX_SCENE_PORTAL_ClipIO(&entity, t.pos)) {
						DrawEERIEInter_Render(entity.obj, t, &entity, invisibility);
					}
				} else {
