...
#if defined(ARXVR_ANDROID_BUILD)
		if(buttonId == Mouse::Button_0 && ARXmenu.mode() != Mode_InGame) {
			// Feed the lower right grip and index trigger through the normal mouse
			// state machine so every menu (including character creation) observes
			// consistent press, hold and release edges. B remains Escape/back.
			bMouseButton[i] = arxvrMenuPrimaryPressed();
		}
#endif
...
#if defined(ARXVR_ANDROID_BUILD)
	if(keyId == Keyboard::Key_Enter && arxvrButtonPressed(ARXVR_BUTTON_RIGHT_A)) {
		return true;
	}
	if(keyId == Keyboard::Key_Escape
	   && (arxvrButtonPressed(ARXVR_BUTTON_RIGHT_B)
	       || arxvrButtonPressed(ARXVR_BUTTON_LEFT_MENU))) {
		return true;
	}
#endif
...
#if defined(ARXVR_ANDROID_BUILD)
	if(keyId == Keyboard::Key_Enter && arxvrButtonPressed(ARXVR_BUTTON_RIGHT_A)) {
		return arxvrButtonNowPressed(ARXVR_BUTTON_RIGHT_A);
	}
	if(keyId == Keyboard::Key_Escape
	   && (arxvrButtonPressed(ARXVR_BUTTON_RIGHT_B)
	       || arxvrButtonPressed(ARXVR_BUTTON_LEFT_MENU))) {
		return arxvrButtonNowPressed(ARXVR_BUTTON_RIGHT_B)
		    || arxvrButtonNowPressed(ARXVR_BUTTON_LEFT_MENU);
	}
#endif
...
#if defined(ARXVR_ANDROID_BUILD)
	if(keyId == Keyboard::Key_Enter) {
		const bool released = arxvrButtonNowReleased(ARXVR_BUTTON_RIGHT_A);
		if(arxvrButtonPressed(ARXVR_BUTTON_RIGHT_A) || released) {
			return released;
		}
	}
	if(keyId == Keyboard::Key_Escape) {
		const bool released = arxvrButtonNowReleased(ARXVR_BUTTON_RIGHT_B)
		                   || arxvrButtonNowReleased(ARXVR_BUTTON_LEFT_MENU);
		if(arxvrButtonPressed(ARXVR_BUTTON_RIGHT_B)
		   || arxvrButtonPressed(ARXVR_BUTTON_LEFT_MENU) || released) {
			return released;
		}
	}
#endif
...
bool Input::actionNowPressed(ControlAction actionId) const {
	...
	if(ARXmenu.mode() == Mode_InGame) {
		if(actionId == CONTROLS_CUST_USE) {
			// A retains the legacy secondary-use / inventory action.
			return arxvrButtonNowPressed(ARXVR_BUTTON_RIGHT_A);
		}
		if(actionId == CONTROLS_CUST_ACTION) {
			// The index trigger keeps Arx's primary action / weapon path. The
			// lower grip is handled separately as a positional, physical grab.
			// A focused world interaction captures the trigger so it cannot also
			// ready or swing a weapon.
			return !arxvrDirectInteractionTriggerCaptured()
...
#if defined(ARXVR_ANDROID_BUILD)
	if(ARXmenu.mode() == Mode_InGame && actionId == CONTROLS_CUST_CROUCH
...
#if defined(ARXVR_ANDROID_BUILD)
		if(ARXmenu.mode() == Mode_InGame) {
			return actionId == CONTROLS_CUST_USE
			     ? arxvrButtonPressed(ARXVR_BUTTON_RIGHT_A)
			     : (!arxvrDirectInteractionTriggerCaptured()
...
bool Input::actionNowReleased(ControlAction actionId) const {
	...
	if(ARXmenu.mode() == Mode_InGame) {
		if(actionId == CONTROLS_CUST_USE) {
			return arxvrButtonNowReleased(ARXVR_BUTTON_RIGHT_A);
		}
		if(actionId == CONTROLS_CUST_ACTION) {
			return !arxvrDirectInteractionTriggerCaptured()
