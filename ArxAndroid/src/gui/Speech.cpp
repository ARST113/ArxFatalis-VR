...
void ARX_SPEECH_Update(bool advanceState) {
	...
	// Legacy cinematic speech scrolls at the extreme edge and was submitted to
	// only one eye. Keep VR subtitles stationary, wrapped and inside the central
	// safe area; ArxGame invokes this renderer once for each eye.
	for(Speech & speech : g_speech) {
		if(speech.text.empty()) {
			continue;
		}
		const s32 left = util::to<s32>(float(g_size.width()) * 0.18f);
		const s32 right = util::to<s32>(float(g_size.width()) * 0.82f);
		const s32 top = util::to<s32>(float(g_size.height()) * 0.68f);
		const s32 bottom = util::to<s32>(float(g_size.height()) * 0.88f);
		const Rect clippingRect(left, top, right, bottom);
		const Vec2f origin(float(left + 12), float(top + 8));
		ARX_UNICODE_DrawTextInRect(hFontInGame, origin + Vec2f(2.f, 2.f),
		                           float(right - 12), speech.text, Color::black,
		                           &clippingRect);
		ARX_UNICODE_DrawTextInRect(hFontInGame, origin, float(right - 12),
		                           speech.text, Color::white, &clippingRect);
	}
	return;
#endif
