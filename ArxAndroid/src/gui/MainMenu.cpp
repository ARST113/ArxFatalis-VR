...
public:
	...
	void init() override {
		...
		addControlRow(CONTROLS_CUST_COLLECT,     "system_menus_options_input_customize_controls_collect");
...
void MainMenu::init() {
	...
	LogInfo << "ArxVR main menu background loaded=" << (m_background != nullptr)
	        << " size=" << (m_background ? m_background->size() : Vec2i(0))
	        << " gpuTexture=" << (m_background ? m_background->m_pTexture : nullptr);
#endif
	
#if defined(ARXVR_ANDROID_BUILD)
	// The Russian "Load/Save" label is much wider than the English original.
	// Move the VR main-menu column left so its complete visible text and hit box
	// remain inside the square per-eye render target.
	Vec2f pos = RATIO_2(Vec2f(300, 100));
#else
...
void MainMenu::render() {
	...
	if(!loggedFirstRender) {
		...
		LogInfo << "ArxVR main menu first render size=" << g_size.width() << "x" << g_size.height()
