...
void ArxGame::managePlayerControls() {
...
#if defined(ARXVR_ANDROID_BUILD)
	// In VR the headset defines the character's forward direction. Consume the
	// incremental head yaw into the player orientation every frame so camera,
	// portal visibility, locomotion and the character never diverge. Pitch stays
	// on the camera and roll is intentionally ignored by AndroidVrInput.
	if(ARXmenu.mode() == Mode_InGame && cinematicIsStopped()
	   && !cinematicBorder.isActive() && !BLOCK_PLAYER_CONTROLS) {
		const float vrBodyTurn = arxvrConsumeBodyYaw();
		if(vrBodyTurn != 0.f) {
			player.angle.setYaw(MAKEANGLE(player.angle.getYaw() + vrBodyTurn));
			player.desiredangle.setYaw(MAKEANGLE(player.desiredangle.getYaw() + vrBodyTurn));
			static float vrTurnTelemetry = 0.f;
			vrTurnTelemetry += vrBodyTurn;
			if(std::abs(vrTurnTelemetry) >= 45.f) {
				LogInfo << "ArxVR body followed head by " << vrTurnTelemetry
				        << " degrees; player yaw=" << player.angle.getYaw();
				vrTurnTelemetry = 0.f;
			}
		}
	}

	const float vrSnapTurn = arxvrConsumeSnapTurn();
	if(vrSnapTurn != 0.f) {
		player.angle.setYaw(MAKEANGLE(player.angle.getYaw() + vrSnapTurn));
	...
	
	{
		...
		if(!NOMOREMOVES && (vrMoveX != 0.f || vrMoveY != 0.f)) {
			const float vrSpeed = arxvrSprintActive() ? 1.6f : 1.f;
			const float movementYaw = MAKEANGLE(player.angle.getYaw() + arxvrHeadYawOffset());
			if(vrMoveY != 0.f) {
				tm += angleToVectorXZ(movementYaw) * (10.f * FD * MoveDiv * vrMoveY * vrSpeed);
				player.m_currentMovement |= vrMoveY > 0.f
				                          ? PLAYER_MOVE_WALK_FORWARD
				                          : PLAYER_MOVE_WALK_BACKWARD;
			}
			if(vrMoveX != 0.f) {
				tm += angleToVectorXZ(movementYaw - 90.f) * (6.f * FD * MoveDiv * vrMoveX * vrSpeed);
				player.m_currentMovement |= vrMoveX > 0.f
				                          ? PLAYER_MOVE_STRAFE_RIGHT
				                          : PLAYER_MOVE_STRAFE_LEFT;
			}
		}
#endif
...
static bool collectItem(Entity * io) {

	if(!io || g_draggedEntity || (io->ioflags & IO_MOVABLE) || !(io->ioflags & IO_ITEM)) {
		return false;
	}

	Entity * container = locateInInventories(io).container;
	if(container && (container->ioflags & IO_SHOP)) {
		// Cannot collect items in shop inventories
		return false;
	}

	if(g_playerInventoryHud.containsPos(DANAEMouse) || ARX_INTERFACE_MouseInBook() || eMouseState == MOUSE_IN_NOTE) {
		return false;
	}
...
void ArxGame::manageKeyMouse() {
	...
	if(ARXmenu.mode() == Mode_InGame) {
		...
		if(!BLOCK_PLAYER_CONTROLS && GInput->actionNowPressed(CONTROLS_CUST_COLLECT)) {
			collectItem(pIO);
		}
