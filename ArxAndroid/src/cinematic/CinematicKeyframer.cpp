...
float GetTrackCurrentFrame() {
	return CKTrack ? CKTrack->currframe : -1.f;
}

int GetTrackEndFrame() {
	return CKTrack ? CKTrack->endframe : -1;
}

int GetTrackFirstFrame() {
	return (CKTrack && !CKTrack->key.empty()) ? CKTrack->key.front().frame : -1;
}

float GetTrackFirstSpeed() {
	return (CKTrack && !CKTrack->key.empty()) ? CKTrack->key.front().speedtrack : -1.f;
}

size_t GetTrackKeyCount() {
	return CKTrack ? CKTrack->key.size() : 0;
}

bool IsTrackPaused() {
	return !CKTrack || CKTrack->pause;
}
