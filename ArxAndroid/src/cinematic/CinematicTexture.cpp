...
std::unique_ptr<CinematicBitmap> CreateCinematicBitmap(const res::path & path, int scale) {
	...
	const bool imageLoaded = cinematicImage.load(filename);
	if(!imageLoaded || !cinematicImage.isValid()) {
		LogError << "ArxVR cinematic bitmap failed: " << filename;
		return { };
	}
	uint64_t pixelSum = 0;
	for(size_t i = 0; i < cinematicImage.getSize(); ++i) {
		pixelSum += cinematicImage.getData()[i];
	}
	LogInfo << "ArxVR cinematic bitmap: " << filename
	        << " size=" << cinematicImage.getWidth() << 'x' << cinematicImage.getHeight()
	        << " bytes=" << cinematicImage.getSize() << " pixelSum=" << pixelSum;
